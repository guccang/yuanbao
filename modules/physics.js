...
function generateLesson(day, age) {
  var random = seeded(day * 5821 + age * 211);
  ...
  if (theme.name === "浮与沉") {
    ...
    var distractors = shuffle(theme.float.concat(theme.sink).filter(function(x) { return x !== testItem; }), random).slice(0, 3);
    ...
    activities = [
      ...
      { title: "哪个有弹性？", hint: "选出能弹回来的东西", visual: "🏀", answer: shuffle(theme.elastic, random)[0], options: shuffle([shuffle(theme.elastic, random)[0], shuffle(theme.inelastic, random)[0], shuffle(theme.elastic, random)[1] || theme.elastic[0], shuffle(theme.inelastic, random)[1] || theme.inelastic[0]], random) },
      { title: age >= 5 ? (elasticItem.split(" ")[1] || elasticItem) + " 被压扁后会怎样？" : "哪个没有弹性？", hint: age >= 5 ? "想一想弹性是什么" : "选出不能弹回来的东西", visual: age >= 5 ? "🤔" : "✗", answer: age >= 5 ? "弹回来" : shuffle(theme.inelastic, random)[0], options: age >= 5 ? ["弹回来", "保持扁的"] : shuffle([shuffle(theme.inelastic, random)[0], shuffle(theme.elastic, random)[0], shuffle(theme.inelastic, random)[1] || theme.inelastic[0], shuffle(theme.elastic, random)[1] || theme.elastic[0]], random) }
    ];
    ...
    activities = [
      ...
      { title: "哪个更光滑？", hint: "选出表面最光滑的东西", visual: "🪞", answer: shuffle(theme.smooth, random)[0], options: shuffle([shuffle(theme.smooth, random)[0], shuffle(theme.rough, random)[0], shuffle(theme.smooth, random)[1] || theme.smooth[0], shuffle(theme.rough, random)[1] || theme.rough[0]], random) },
      { title: age >= 5 ? "在滑梯上为什么滑得快？" : "哪个更粗糙？", hint: age >= 5 ? "因为表面很光滑，摩擦力小" : "选出表面最粗糙的东西", visual: age >= 5 ? "🛝" : "🧗", answer: age >= 5 ? "因为表面光滑" : shuffle(theme.rough, random)[0], options: age >= 5 ? ["因为表面光滑", "因为表面粗糙"] : shuffle([shuffle(theme.rough, random)[0], shuffle(theme.smooth, random)[0], shuffle(theme.rough, random)[1] || theme.rough[0], shuffle(theme.smooth, random)[1] || theme.smooth[0]], random) }
    ];
    ...
    activities = [
      ...
      { title: "哪个是工具？", hint: "选出能帮我们省力的东西", visual: "⚙️", answer: shuffle(theme.tool, random)[0], options: shuffle([shuffle(theme.tool, random)[0], shuffle(theme.notTool, random)[0], shuffle(theme.tool, random)[1] || theme.tool[0], shuffle(theme.notTool, random)[1] || theme.notTool[0]], random) },
      { title: age >= 5 ? "轮子能帮我们做什么？" : "哪个不是工具？", hint: age >= 5 ? "想一想轮子的作用" : "选出不能帮我们省力的东西", visual: age >= 5 ? "🛞" : "🤔", answer: age >= 5 ? "让东西更容易移动" : shuffle(theme.notTool, random)[0], options: age >= 5 ? ["让东西更容易移动", "让东西变重"] : shuffle([shuffle(theme.notTool, random)[0], shuffle(theme.tool, random)[0], shuffle(theme.notTool, random)[1] || theme.notTool[0], shuffle(theme.tool, random)[1] || theme.tool[0]], random) }
    ];
  } else { // 颜色混合
    ...
    activities = [
      ...
      { title: mix.a + " 加 " + mix.b + " 会变成什么颜色？", hint: "两种颜色混合在一起", visual: mix.a + " + " + mix.b, answer: mix.result, options: shuffle([mix.result].concat(allColors.filter(function(c) { return c !== mix.result; }).slice(0, 3)), random) },
      { title: "橙色是怎么变出来的？", hint: "想一想，哪两种颜色混合？", visual: "🟠", answer: "红色加黄色", options: shuffle(["红色加黄色", "红色加蓝色", "蓝色加黄色", "白色加黑色"], random) },
      { title: "绿色是怎么变出来的？", hint: "想一想，哪两种颜色混合？", visual: "🟢", answer: "蓝色加黄色", options: shuffle(["蓝色加黄色", "红色加黄色", "红色加蓝色", "白色加黑色"], random) },
      { title: age >= 5 ? dayColorName + " 是什么颜色？" : "哪个是 " + dayColorName + "？", hint: "仔细观察颜色", visual: dayColor === "red" ? "🔴" : dayColor === "yellow" ? "🟡" : dayColor === "blue" ? "🔵" : dayColor === "orange" ? "🟠" : dayColor === "green" ? "🟢" : "🟣", answer: dayColorName, options: age >= 5 ? shuffle(allColors.slice(), random) : shuffle([dayColorName].concat(allColors.filter(function(c) { return c !== dayColorName; }).slice(0, 3)), random) }
    ];
  }
